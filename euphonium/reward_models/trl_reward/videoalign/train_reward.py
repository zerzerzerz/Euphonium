...
def create_model_and_processor(
    ...
    qwenvl_path = os.environ.get('QWENVL_PATH', None)
    if not qwenvl_path:
        tmp = 'path/to/Qwen2-VL-2B-Instruct'  # please set your default path here
        print(f"===> Using default Qwen2-VL path: {tmp}")
        qwenvl_path = tmp
    print(f"===> Loading Qwen2-VL from {qwenvl_path}")

    # create processor and set padding
    processor = AutoProcessor.from_pretrained(qwenvl_path,
                                              padding_side="right",
                                              cache_dir=cache_dir)
