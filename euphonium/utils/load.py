...
from ..models.hunyuan.modules.models import (
    HYVideoDiffusionTransformer, MMDoubleStreamBlock, MMSingleStreamBlock)
from ..models.hunyuan.text_encoder import TextEncoder
from ..models.hunyuan.vae.autoencoder_kl_causal_3d import \
    AutoencoderKLCausal3D
from ..models.hunyuan_hf.modeling_hunyuan import (
...
def load_transformer(
    ...
    if model_type == "hunyuan_hf":
        ...
        # If trained weights are specified, load and replace (check if prompt is empty string)
        if dit_model_name_or_path and dit_model_name_or_path.strip():
            main_print(f"Loading trained weights from: {dit_model_name_or_path}")
            
            # Determine whether to load from directory or single file
            if os.path.isfile(dit_model_name_or_path):
                # If it's a .safetensors or .bin file
                if dit_model_name_or_path.endswith('.safetensors'):
                    from safetensors.torch import load_file
                    state_dict = load_file(dit_model_name_or_path)
                else:
                    state_dict = torch.load(dit_model_name_or_path, map_location="cpu", weights_only=True)
                
                # Load weights into model
                missing_keys, unexpected_keys = transformer.load_state_dict(state_dict, strict=False)
                if missing_keys:
                    main_print(f"Missing keys when loading trained weights: {missing_keys}")
                if unexpected_keys:
                    main_print(f"Unexpected keys when loading trained weights: {unexpected_keys}")
                main_print(f"Successfully loaded trained transformer weights from {dit_model_name_or_path}")
            else:
                # If it's a directory, use from_pretrained
                transformer = HunyuanVideoTransformer3DModel.from_pretrained(
                    dit_model_name_or_path,
                    torch_dtype=master_weight_type,
                )
        else:
            main_print(f"Using pretrained model weights from: {pretrained_model_name_or_path}/transformer")
        ...
        # Check if dit_model_name_or_path is valid
        if not dit_model_name_or_path or not dit_model_name_or_path.strip():
            raise ValueError("For hunyuan model type, dit_model_name_or_path must be provided and cannot be empty")
        transformer = load_hunyuan_state_dict(transformer,
                                              dit_model_name_or_path)
        if master_weight_type == torch.bfloat16:
            transformer = transformer.bfloat16()
    else:
        raise ValueError(f"Unsupported model type: {model_type}. Only 'hunyuan', 'hunyuan_hf' are supported.")
    return transformer
...
def load_text_encoder(model_type, pretrained_model_name_or_path, device):
    if model_type == "hunyuan" or model_type == "hunyuan_hf":
...
def get_no_split_modules(transformer):
    if isinstance(transformer, HunyuanVideoTransformer3DModel):
        ...
        raise ValueError(f"Unsupported transformer type: {type(transformer)}. Only Hunyuan models are supported.")
