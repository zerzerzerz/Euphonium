...
def save_checkpoint(transformer, rank, output_dir, step, epoch, optimizer=None, lr_scheduler=None):
    ...
    if rank <= 0:
        ...
        if optim_state is not None:
            optimizer_path = os.path.join(save_dir, "optimizer.pt")
            torch.save(optim_state, optimizer_path)
            main_print(f"  ✓ Saved optimizer state")
        ...
        if lr_scheduler is not None:
            ...
            main_print(f"  ✓ Saved lr_scheduler state")
        ...
        main_print(f"  ✓ Saved training state (step={step}, epoch={epoch})")
        
    main_print(f"--> checkpoint saved at step {step}")


def load_checkpoint(transformer, optimizer, lr_scheduler, checkpoint_dir, rank):
    """
    Load a complete training checkpoint, including model, optimizer, lr_scheduler, and training state.
    
    Args:
        transformer: FSDP-wrapped model.
        optimizer: Optimizer.
    ...
    Returns:
        tuple: (transformer, optimizer, lr_scheduler, step, epoch)
    """
    main_print(f"--> Loading checkpoint from {checkpoint_dir}")
    ...
    with FSDP.state_dict_type(
        ...
        main_print(f"  ✓ Loaded model weights")
        ...
        if os.path.exists(optim_path):
            ...
            main_print(f"  ✓ Loaded optimizer state")
        else:
            main_print(f"  ⚠ Optimizer state not found, starting with fresh optimizer")
    
    # Load lr_scheduler state
    scheduler_path = os.path.join(checkpoint_dir, "scheduler.pt")
    if os.path.exists(scheduler_path) and lr_scheduler is not None:
        scheduler_state = torch.load(scheduler_path, weights_only=False)
        lr_scheduler.load_state_dict(scheduler_state)
        main_print(f"  ✓ Loaded lr_scheduler state")
    else:
        main_print(f"  ⚠ Scheduler state not found, starting with fresh scheduler")
    ...
    if os.path.exists(training_state_path):
        ...
        main_print(f"  ✓ Loaded training state (step={step}, epoch={epoch})")
    else:
        # Attempt to extract step from directory name
        try:
            parts = checkpoint_dir.rstrip("/").split("-")
            step = int(parts[-2]) if len(parts) >= 2 else int(parts[-1])
            epoch = int(parts[-1]) if len(parts) >= 3 else 0
            main_print(f"  ⚠ Training state file not found, extracted from path: step={step}, epoch={epoch}")
        except:
            step = 0
            epoch = 0
            main_print(f"  ⚠ Could not extract training state, starting from step=0")
    
    main_print(f"--> Successfully loaded checkpoint from step {step}")
    return transformer, optimizer, lr_scheduler, step, epoch
