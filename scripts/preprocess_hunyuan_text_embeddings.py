...
from euphonium.utils.load import load_text_encoder, load_vae
from diffusers.video_processor import VideoProcessor
from tqdm import tqdm
import re

# Get the local rank from the environment variable set by the launcher
...
def main(args):
    # RANK is the global rank across all nodes (0 to world_size-1)
    # LOCAL_RANK is the rank within the current node (0 to num_gpus_per_node-1)
    ...
    # Use LOCAL_RANK for CUDA device (0-7 per node), not global RANK
    torch.cuda.set_device(local_rank)
        ...
        with torch.inference_mode():
            with torch.autocast("cuda"):
                ...
                for idx, video_name in enumerate(data["filename"]):
                    ...
                    # Save absolute paths for direct loading
